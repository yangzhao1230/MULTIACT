...
from diffusion.utils import create_diffusion, ClassifierFreeSampleModel
...
class Tester():
    def __init__(self, cfg, model=None, test_train_loader=None, test_logger=None):
        ...
        if cfg.model == 'MDM' and cfg.guidance_param != 1:
    ...
    def test(self):
        ...
        for rep in tqdm(range(test_num_rep)):
            print(f'****************rep:{rep}****************')
    ...
    def generate_output(self, split="unseen"):
        ...
        for itr, inputs in enumerate(loader):
            ...
            with torch.no_grad():
                if self.cfg.model == 'MDM':
                    out = self.diffusion_sample(inputs)
